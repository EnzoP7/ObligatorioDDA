...
public class Jugadores extends Persona {
  ...
  public static Jugadores fromString(String linea) {
    ...
    if (BuscarJugador(id) == null) {
      Jugadores elJugador = new Jugadores(
          ...
          Integer.parseInt(separateDataValue(splitData[6])));
      listaPersonas.add(elJugador);
      return elJugador;
    }
    return null;
  }
  ...
  public static Jugadores BuscarJugador(int pId) {
    for (Jugadores jugador : listaJugadores) {
      if (jugador.getId() == pId) {
        return jugador;
      }
    }
    return null;
  ...
  public static void verPagos() {
    ...
    for (Jugadores elPlayer : listaJugadores) {
      ...
      System.out.println(
          Nombre + " " + Apellido + ": Partidos Jugados: " + partidosJugados + "  ---- TOTAL: $" + PagoTotal);
    }
  }
      ...
      for (Jugadores elPlayer : listaJugadores) {
        if (elPlayer.getId() == elId) {
          ...
          if (jugo) {
            ...
            System.out.println("Jugador " + elPlayer.getNombre() + " " + elPlayer.getApellido()
                + " ha sido Eliminado");
            return;
          }
        }
